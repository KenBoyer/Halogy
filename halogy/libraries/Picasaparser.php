<?php
/*
 * Picasaparser (originally GalleryFromPicasa) get datas of galleries, pictures from Picasaweb
 *
...
class Picasaparser
{
     ...
     *
     */
    public function getPictures($userid, $albumid, $thumbsize = "", $picturesize = "")
    {
        $pictures = array();
        ...
        $pictures['albumtitle'] = $this->result["FEED"]["TITLE"]["value"];
        $id                     = 0;
        foreach ($this->getKeys("ENTRY") AS $key => $value)
        {
            foreach ($this->result["FEED"][$value] AS $gphotokey => $gphotovalue)
