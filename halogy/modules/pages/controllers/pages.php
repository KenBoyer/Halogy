...
class Pages extends MX_Controller {
	...
	function view($page, $sendthru = '', $module = FALSE, $return = FALSE)
	{
		...
		elseif ($pagedata = $this->core->get_active_page($page))
		{
			...
			// enable cache (parameter is the number of minutes you wish the page to remain cached between refreshes)
			$this->output->cache(5);
			// set to 4 hours (was set to 2 weeks, which was a bit too long)
			$this->output->set_header('Expires: ' . gmdate('D, d M Y H:i:s', time()+1*4*60*60) . ' GMT');
	...
	function search($query = '')
	{
		...
		// set the query based on whether it's a parameter or a POST
		$query = ($query) ? $query : strip_tags($this->input->get_post('query', TRUE));

		$results = $this->pages->search($query);
		...
		if ($results)
		{
			...
			{
				$output['page:results'][] = array(
					'result:link' => site_url($result['uri']),
		...
		// Need to set up highlights here, so they will be available when block content is integrated
		$this->session->set_userdata('highlight_phrase', $query);

		// set title
		$output['page:title'] = 'Search site'.(($this->site->config['siteName']) ? ': '.$this->site->config['siteName'] : '');
		$output['page:heading'] = 'Search results for "'.$output['query'].'"';
		...
		// TBD: Add a template for "page_search"?
		$this->view('pages-search', $output);
	}
}