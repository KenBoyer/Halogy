...
//				console_debug(__FILE__.':'.__FUNCTION__.": _FILES=", $_FILES);
	...
	function add_cat()
	{
		...
		if (count($_POST))
		{
			if ($this->core->check_errors())
			{
				...
				$catStart = DateTime::createFromFormat('d M Y', $this->input->post('catStart'));
				if (is_object($catStart))
				{
					$this->core->set['catStart'] = date("Y-m-d", strtotime($catStart->format('Y-m-d')));
				}
				$catStop = DateTime::createFromFormat('d M Y', $this->input->post('catStop'));
				if (is_object($catStop))
				{
					$this->core->set['catStop'] = date("Y-m-d", strtotime($catStop->format('Y-m-d')));
				}
	...
	function deactivate_review($reviewID)
	{
		// check permissions for this page
		if (!in_array('shop_reviews', $this->permission->permissions))
		{
		...
		if ($this->shop->deactivate_review($reviewID))
		{
			redirect('/admin/shop/reviews');
		}
	}
