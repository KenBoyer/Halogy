...
(function ($) {
	...
	function showForm(a) {
		...
		$(a).addClass('active');
		if ($url != '#') {
			$('div.hidden-form').load($url, function () {
				$('div.hidden-form:visible').slideUp(200);
				$('div.hidden-form').slideDown(200, function () {
	...
	function hideForm(a) {
		$.scrollTo(0, 200);
		$('.showform').removeClass('active');
		$('div.hidden-form:visible').slideUp(200, function () {
			$('div.hidden-form').html('')
		})
	}
	function checkajax() {
		if (updated == 0) {
	...
	function toggle() {
		var a = $('#controls:hidden-form').length;
