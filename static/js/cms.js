/*! Halogy v.1.2 http://halogy.com/ | http://halogy.com/license */
$(function() {
    $.fn.resizer = function () {
        var b = !($.browser.msie || $.browser.opera);

        function doResize(e) {
            e = e.target || e;
            var a = e.value.length,
                ewidth = e.offsetWidth;
            if (a != e.valLength || ewidth != e.boxWidth) {
        ...
        this.each(function () {
            ...
            this.expandMin = 100;
            this.expandMax = (p ? parseInt('0' + p[2], 10) : 99999);
            doResize(this);
            if (!this.Initialized) {
                this.Initialized = true;
                $(this).css("padding-top", 0).css("padding-bottom", 0);
                $(this).bind("keyup", doResize).bind("focus", doResize)
            }
        });
        return this
    }
...
$(function() {
    ...
    function edit(a) {
        ...
        l = $(b).contents().find('textarea.code');
        var g = $(l).val();
        if ($(b).width() > 0) {
            $(b).width($(b).width())
        } else {
    ...
    function insertimage(a) {
        $(l).focus();
        $(l).insertAtCaret('{image:' + $(a).attr('title') + '}');
        hidebrowser()
    }
    ...
    function insertfile(a) {
        $(l).focus();
        $(l).insertAtCaret('{file:' + $(a).attr('title') + '}');
        hidebrowser()
    }
    ...
    $('textarea.code').keydown(function (e) {
        if (e.keyCode == 9) {
            ...
            cancel($(this).closest('div.halogycms_edit').find('a.halogycms_cancelbutton'))
        }
    });
